...
-- Source: docs/spec/01-database-schema.md (assembled from the 22 architecture
-- decisions in .scratch/coresearch-saas-architecture/map.md). This file
-- reorders the tables from that doc into dependency order and adds the
-- research_entities -> agent_messages foreign key as a separate ALTER TABLE
-- at the end, since agent_messages didn't exist yet at the point in the spec
-- doc where research_entities was defined — a real ordering bug caught while
-- assembling this migration, not a new decision.
...
-- ── 2. Agent Worker (created before Research Domain: research_entities
--      references agent_messages via source_candidate_id) ────────────────
...
CREATE TABLE research_entities (
  ...
  source_candidate_id uuid NULL,  -- FK added below, after agent_messages exists
  branch_id uuid NULL,
