...
- [`canvas_state` 的规范存储形态](issues/18-canvas-state-canonical-storage-model.md)：`canvas_state` 整体快照概念被取代，统一 `canvas_nodes` 表覆盖原生+Research-managed 节点，`canvas_projections` 是 1:1 binding 不是平行节点体系；几何直接放 `canvas_nodes`；研究关系边是投影器计算的虚拟边，不持久化。详见 [ADR 0008](../../docs/adr/0008-canvas-canonical-storage-unified-nodes.md)（更正 ADR 0005 的 `canvas_state` 措辞）。
