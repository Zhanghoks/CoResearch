...
- [ ] `MERGE_NODE_DATA` 改 `RESEARCH_OWNED_DATA_KEYS` 里的字段，返回 `applied:false, reason:'invalid-scope'`
- [ ] `MERGE_NODE_DATA` 改 `native_data`（画布层批注）成功
- [ ] `executeFromRequest()` 和 `executeAsProjector()` 是两个不同的函数导出，不是同一入口加参数——有一条测试直接调用 `executeFromRequest` 传一个伪造的 `source:'system'` 标签，验证依然被拒（因为判断依据是函数入口不是参数）
- [ ] 模拟一次实体 revision 变化后，投影器刷新同一个 `nodeId` 的渲染内容，`canvas_layout`（用户几何）不受影响，没有产生第二个 `canvas_nodes` 行
